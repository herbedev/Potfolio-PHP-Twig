...
$(window).on('load', function () {
  ...
  /* 07. Box */
  new Accordion(
    ['.service', '.info', '.work-process', '.form-block', '.contact-block'],
    {
      showItem: true,
    }
  )
...
$(document).ready(function () {
  ...
  $(document).on('click', '.menu-item:not(.js-btn)', function () {
    ...
    $(document).off('click', '.content', closeNotFocusedMenu)
    setTimeout(function () {
      $('.menu')[0].className = $('.menu')[0].className.replace(
        /\bpage-active-.*\b/gi,
        ''
      )
  ...
  ajaxPortfolioSetup($('.portfolio-items .ajax-link'), $('.ajax-portfolio'))
